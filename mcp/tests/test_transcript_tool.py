...
async def test_the_next_hint_repeats_the_span_its_offset_counts_in(deps) -> None:
    ...
    # Rounding 5.8004 to 5.8 would pull in the cue ending at 5.8 and shift the offset.
    _, payload = await read(deps, video_id=VIDEO, t_start=5.8004, t_end=10, limit=1)
    assert "offset=1 t_start=5.8004 t_end=10 continues" in payload["next"]
