...
The selectable alternative to the default ``qwen3-embedding``: MIT, 1024 dims,
8k context, and — the reason it is kept — a sparse/lexical head no dense-only
model replaces, which for CLI flags, error codes and proper nouns is worth
something. Only the dense head is exposed here.
...
class BGEM3Backend(BaseBackend):
    ...
    def infer(
        ...
        # bge-m3 is symmetric — no instruction prefixes — so ``input_type`` is
