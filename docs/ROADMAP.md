...
### 2. `failing` never clears itself — `following.md` §11 Q4
...
### 3. F11 — no tool parameter has a description
...
### 4. `cancel-job` and `delete-video` — F12's unfinished half
...
### 5. A DOM-level test harness for the public page
...
### 6. F1 — structured OCR, the big one
...
### 7. The channel-count quirk — needs re-diagnosis before it is a ticket
...
### 8. A wrong method on `/api/*` answers 404, not 405
