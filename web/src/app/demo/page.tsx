...
// The demo — the working corpus, at `/demo` (demo-site.md §1). Static shell;
// the mode (search or ask) is read from the URL inside the boundary, the box is
// the one Client Component in search mode, and the results are a Server
// Component that re-renders for each URL.
...
async function Mode({ searchParams }: Pick<PageProps<"/demo">, "searchParams">) {
  ...
  return (
    <>
      ...
      <p className={styles.switch}>
        <Link href={q ? `/demo?ask=1&q=${encodeURIComponent(q)}` : "/demo?ask=1"}>
          ask instead →
        </Link>
      </p>
      <Results q={q} sp={sp} />
  ...
  if (results.length === 0) {
    return (
      <div className={styles.empty}>
        ...
        {type !== "all" ? (
          <p>
            <Link href={`/demo?q=${encodeURIComponent(q)}`}>Search all</Link>
          </p>
        ) : null}
        <p>
          <Link href="/demo">Try one of the examples.</Link>
        </p>
      </div>
    );
  }
  ...
  return (
    <section className={styles.results} aria-label="Results">
      ...
      <nav className={styles.pager} aria-label="Pages">
        {pageOffset > 0 ? (
          <Link href={`/demo?${query}&offset=${Math.max(0, pageOffset - pagination.limit)}`}>
            ← previous
          </Link>
        ) : (
          <span />
        ...
        {pagination.has_more && offset === pageOffset ? (
          <Link href={`/demo?${query}&offset=${pageOffset + pagination.limit}`}>
            more results →
          </Link>
        ) : null}
      </nav>
  ...
  return (
    <div className={styles.examples}>
      ...
      <ul className={styles.chips}>
        {EXAMPLES.map((q) => (
          <li key={q}>
            <Link href={`/demo?q=${encodeURIComponent(q)}`} className={styles.chip}>
              {q}
            </Link>
          </li>
        ))}
